...
inline HeaderGeneral ProcessHeaderGeneralFields(HeaderGeneralFields in) {
  HeaderGeneral out;
  ...
  // Start Date
  {
    ...
    }
    {
      ...
      auto [p2, e2] =
          std::from_chars(startdate.data() + 3, startdate.data() + 5, month);
      auto [p3, e3] =
          std::from_chars(startdate.data() + 6, startdate.data() + 8, year);
  ...
  // Start Time
  {
    ...
    }
    {
      ...
      auto [p2, e2] =
          std::from_chars(starttime.data() + 3, starttime.data() + 5, minute);
      auto [p3, e3] =
          std::from_chars(starttime.data() + 6, starttime.data() + 8, second);
      if (e1 != std::errc{} || e2 != std::errc{} || e3 != std::errc{} ||
  ...
  // Header Size
  {
    out.m_headerSize = detail::ParseInt(
        in.m_headerSize(), GetError(FileErrc::FileContainsFormatErrors));
  }
  // Reserved
  {
  ...
  // Datarecords in File
  {
    out.m_datarecordsFile = detail::ParseLongLong(
        in.m_datarecordsFile(), GetError(FileErrc::FileContainsFormatErrors));
  ...
  {
    double duration =
        detail::ParseDouble(in.m_datarecordDuration(),
                            GetError(FileErrc::FileContainsFormatErrors));
    out.m_datarecordDuration = duration;
    if (duration < 0) {
      throw std::invalid_argument(GetError(FileErrc::FileContainsFormatErrors));
    }
    out.m_detail.m_fileDuration =
        out.m_datarecordDuration * out.m_datarecordsFile;
  }
  // Number of signals
  {
    ...
    // Patient Name
    {
      ...
      for (size_t i = 0; i < fields.size(); i++) {
        ...
        case 1: // Sex
          ...
          break;
        case 2: // Birthdate in dd-MMM-yyyy format using the English 3-character
      ...
      for (size_t i = 0; i < fields.size(); i++) {
        ...
        if (str != "X") {
          ...
          case 0: // The text 'Startdate'.
            ...
            break;
          case 1: // The startdate itself in dd-MMM-yyyy format using the
                  // English 3-character abbreviations of the month in capitals:
            ...
            break;
          case 2: // The hospital administration code of the investigation, i.e.
                  // EEG number or PSG number.
            details.m_admincode = str;
            break;
          case 3: // A code specifying the responsible investigator or
                  // technician.
  ...
  out.m_detail.m_patientAdditional =
      detail::ReduceString(out.m_detail.m_patientAdditional);
  ...
  out.m_detail.m_recordingAdditional =
      detail::ReduceString(out.m_detail.m_recordingAdditional);
