...
inline HeaderGeneralFields ProcessHeaderGeneral(HeaderGeneral in) {
  ...
  // Datarecord Duration
  {
    out.m_datarecordDuration(
        detail::to_string_decimal(in.m_datarecordDuration));
  }
  // Number of signals
  {
  ...
  if (IsPlus(in.m_version)) {
    // Patient Name
    {
      ...
      // Code
      fields.push_back(
          in.m_detail.m_patientCode.empty() ? "X" : in.m_detail.m_patientCode);
      // Sex
      fields.push_back(in.m_detail.m_gender.empty() ? "X"
                                                    : in.m_detail.m_gender);
      ...
      // 2-AUG-1951 is not.
      fields.push_back(
          in.m_detail.m_birthdate.empty() ? "X" : in.m_detail.m_birthdate);
      // The patients name.
      fields.push_back(
          in.m_detail.m_patientName.empty() ? "X" : in.m_detail.m_patientName);
    ...
    // Recording
    {
      ...
      fields.push_back(std::format("{:02d}-{}-{}", day ? day : 1,
                                   ...
                                   year ? year : 1984));
      // The hospital administration code of the investigation, i.e. EEG number
      // or PSG number.
      fields.push_back(
          in.m_detail.m_admincode.empty() ? "X" : in.m_detail.m_admincode);
      // A code specifying the responsible investigator or technician.
      fields.push_back(
          in.m_detail.m_technician.empty() ? "X" : in.m_detail.m_technician);
      // A code specifying the used equipment.
      fields.push_back(
          in.m_detail.m_equipment.empty() ? "X" : in.m_detail.m_equipment);
      // Additional information.
      std::string info;
