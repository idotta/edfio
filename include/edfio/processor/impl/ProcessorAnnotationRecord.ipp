...
namespace edfio
{
	...
	ProcessorAnnotationRecord::TypeO ProcessorAnnotationRecord::operator << (TypeI in)
	{
		TypeO ou;
		auto& record = in();
	...
	ProcessorAnnotationRecord::TypeI ProcessorAnnotationRecord::operator >> (TypeO in)
	{
		TypeI ou(m_recordSize);

		auto it = ou().begin();
		for (auto& annot : in)
		{
			char signal = annot.m_start >= 0 ? '+' : '-';
			// First annotation is timestamp
			if (it == ou().begin())
			{
				*it++ = signal;
				if (annot.m_start != 0)
				{
					auto tmp = std::to_string(static_cast<double>(annot.m_start) / TIME_DIMENSION);
