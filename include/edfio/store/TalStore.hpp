...
class TalStore
    : public Store<Record<char>::VectorType, Record<char>::VectorType const *,
                   Record<char>::VectorType const &, const Record<char>,
                   std::bidirectional_iterator_tag> {
  using base_store =
      Store<Record<char>::VectorType, Record<char>::VectorType const *,
            Record<char>::VectorType const &, const Record<char>,
            std::bidirectional_iterator_tag>;
...
public:
  ...
  protected:
    iterator(const TalStore *context, size_type off)
        : m_offset(off), m_context(context) {
      if (m_offset == 0)
        ++*this;
    }
  ...
  TalStore(stream_type &stream) : store_type(stream) {}

  iterator begin() const { return iterator(this, 0); }
  const_iterator cbegin() const { return begin(); }
  iterator end() const { return iterator(this, m_stream.Size()); }
...
protected:
  reference getR() const { return m_value; }

  pointer getP() const { return &m_value; }
