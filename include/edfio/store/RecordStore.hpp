...
class RecordStore
    : public Store<Record<char>, Record<char> const *, Record<char> const &,
                   std::ifstream, std::random_access_iterator_tag> {
  using base_store =
      Store<Record<char>, Record<char> const *, Record<char> const &,
            std::ifstream, std::random_access_iterator_tag>;
...
public:
  ...
  public:
    ...
    iterator &operator+=(difference_type n) {
      if (!m_context)
        throw std::invalid_argument("Invalid context");
      if (n < 0)
        return *this -= static_cast<size_type>(-n);
  ...
  RecordStore(stream_type &stream, size_type recordSize, size_type storeSize,
              std::streamoff headerOffset)
      : store_type(stream), m_recordSize(recordSize), m_storeSize(storeSize),
